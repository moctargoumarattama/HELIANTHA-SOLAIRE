...
class ProductSelector:
    ...
    def select_pump(self, target_power_kw: float, *, flow_m3_h: float | None = None, hmt_m: float | None = None) -> dict[str, Any]:
        candidates = []
        for product in self.category_products("pumps"):
            power_kw = as_float(spec_value(product, "power_kw", "rated_power_kw"))
            min_flow = as_float(spec_value(product, "min_flow_m3_h", "flow_m3_h"))
            max_flow = as_float(spec_value(product, "max_flow_m3_h"))
            min_head = as_float(spec_value(product, "min_head_m", "hmt_m"))
            max_head = as_float(spec_value(product, "max_head_m"))
            checks = []
            warnings_list = []
            if power_kw is None or power_kw <= 0:
                checks.append(check("PUMP_POWER_MISSING", "Puissance pompe", "failed", "La puissance pompe manque dans le catalogue."))
            else:
                checks.append(check(
                    "PUMP_TARGET_POWER",
                    "Puissance pompe",
            ...
            if flow_m3_h is not None and min_flow is not None and max_flow is not None:
                flow_ok = min_flow <= flow_m3_h <= max_flow
                checks.append(check(
                    "PUMP_FLOW_RANGE",
                    "Debit pompe",
                    "passed" if flow_ok else "failed",
                    "Le debit cible est dans la plage produit." if flow_ok else "Le debit cible sort de la plage produit.",
                    expected=f"{min_flow}..{max_flow}",
                    actual=flow_m3_h,
                ))
            elif flow_m3_h is not None:
                checks.append(check("PUMP_FLOW_DATA_MISSING", "Debit pompe", "manual", "La plage de debit produit est incomplete."))
                warnings_list.append(warning("PUMP_FLOW_VALIDATION_REQUIRED", "Le debit catalogue doit etre confirme pour la pompe retenue."))
            if hmt_m is not None and min_head is not None and max_head is not None:
                head_ok = min_head <= hmt_m <= max_head
                checks.append(check(
                    "PUMP_HEAD_RANGE",
                    "HMT pompe",
                    "passed" if head_ok else "failed",
                    "La HMT cible est dans la plage produit." if head_ok else "La HMT cible sort de la plage produit.",
                    expected=f"{min_head}..{max_head}",
                    actual=hmt_m,
                ))
            elif hmt_m is not None:
                checks.append(check("PUMP_HEAD_DATA_MISSING", "HMT pompe", "manual", "La plage HMT produit est incomplete."))
                warnings_list.append(warning("PUMP_HEAD_VALIDATION_REQUIRED", "La plage HMT catalogue doit etre confirmee pour la pompe retenue."))
            compatibility = compatibility_result(checks, warnings_list)
            candidates.append(self._candidate(
                component="pump",
                product=product,
                quantity=1 if power_kw else 0,
                compatibility=compatibility,
                reasons=[f"Puissance catalogue: {power_kw:.2f} kW." if power_kw else "Puissance produit manquante."],
                metrics={"target_power_kw": float(target_power_kw), "catalog_power_kw": power_kw, "flow_m3_h": flow_m3_h, "hmt_m": hmt_m},
                oversizing=max(0.0, (power_kw or 0) - float(target_power_kw)),
            ))
        return self._select_best("pump", candidates, "Puissance pompe cible")

    def select_pump_drive(self, pump: dict[str, Any] | None, target_power_kw: float, *, pv_voltage_v: float | None = None) -> dict[str, Any]:
        candidates = []
        for product in self.category_products("drives"):
            compatibility = self.compatibility.check_pump_drive(pump, product, pv_voltage_v=pv_voltage_v)
            power_kw = as_float(spec_value(product, "motor_power_kw", "rated_power_kw", "power_kw"))
            reasons = [f"Puissance variateur catalogue: {power_kw:.2f} kW." if power_kw else "Puissance variateur non documentee."]
            candidates.append(self._candidate(
                component="pump_drive",
                product=product,
                quantity=1 if power_kw else 0,
                compatibility=compatibility,
                reasons=reasons,
                metrics={"target_power_kw": float(target_power_kw), "catalog_power_kw": power_kw},
                oversizing=max(0.0, (power_kw or 0) - float(target_power_kw)),
            ))
        return self._select_best("pump_drive", candidates, "Puissance variateur cible")
